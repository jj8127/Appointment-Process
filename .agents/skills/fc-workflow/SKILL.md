---
name: fc-workflow
description: "fc-onboarding-app 프로젝트 전용. FC 온보딩 워크플로우 이해 및 상태 관리 가이드. FC 상태, 위촉 프로세스, 권한 관련 질문 시 사용."
---

Applies to `D:/hanhwa/fc-onboarding-app` and its verified worktrees. Read the governing workspace/repository instructions and current product contracts before applying the examples below. Use the current shell and available Codex tools; this document does not configure tools or grant external-action permission.

The source document is preserved in the reference import. Current code, migrations, and contracts determine whether its examples still apply.
...
| `temp_id_issued` | 임시 ID 발급됨 | 보증 보험 동의 승인 |
| `consent_approved` | 보증 보험 동의 승인됨 | 시험 등록 → 서류 승인 |
