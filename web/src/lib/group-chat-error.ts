...
export function classifyGroupChatError(input: unknown): GroupChatUserError {
  ...
  if (code === 'not_completed' || hasAny(normalizedMessage, ['본등록', 'signup_completed'])) {
    return makeError({
      ...
      message: '참여 조건 제한입니다. 본등록이 완료된 계정만 단톡방에 참여할 수 있습니다.',
      severity: 'warning',
      code,
      status,
    });
  ...
  if (code === 'request_board_designer_only' || code === 'manager_referral_shadow') {
    return makeError({
      title: '단톡방 참여 대상 아님',
      message: '설계의뢰 전용 계정은 단톡방에 참여할 수 없습니다.',
      severity: 'warning',
      code,
      status,
    });
  ...
  if (code === 'inactive_account') {
    return makeError({
      title: '비활성 계정',
      message: '비활성화된 계정은 단톡방에 참여할 수 없습니다.',
      severity: 'warning',
      code,
      status,
    });
  ...
  if (status === 403 || code === 'forbidden') {
    return makeError({
      title: '단톡방 참여 제한',
      message: '참여 조건 제한입니다. 본등록 완료, 활성 계정 여부, 단톡방 대상 여부를 확인해주세요.',
      severity: 'warning',
      code,
      status,
    });
  ...
  if (status === 404 || code === 'not_found' || code === 'group_chat_account_not_found') {
    return makeError({
      title: '단톡방 계정 확인 필요',
      message: normalizedMessage || '단톡방에 연결된 계정을 찾을 수 없습니다.',
      severity: 'warning',
      code,
      status,
    });
