...
where post.category_id = category.id
  and category.slug = 'insurance-news';
...
from target, public.board_categories category
where post.category_id = category.id
  and category.slug not in ('notice', 'education', 'general', 'garam-pick');

update public.board_categories
set
  is_active = false,
  updated_at = now()
where slug not in ('notice', 'education', 'garam-pick', 'general');
