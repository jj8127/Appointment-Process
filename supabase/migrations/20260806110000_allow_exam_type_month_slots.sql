-- The mobile client has separate life/nonlife application flows.  The durable
-- slot key is therefore (fc_id, exam_month, exam_type), not just month.
-- Keep every prior authorization, payment-proof, lifecycle, and audit check
-- intact while correcting the production v3 RPC that still read the old key.
...
begin
     ...
     where profile.id = p_fc_id
       ...
       and (
         ...
         or (
           ...
           and not exists (
             select 1 from public.manager_accounts manager_target
              where manager_target.active = true
                and regexp_replace(coalesce(manager_target.phone, ''), '[^0-9]', '', 'g')
                  = regexp_replace(p_resident_id, '[^0-9]', '', 'g')
           )
           and not exists (
             select 1 from public.admin_accounts staff_target
  ...
  if p_actor_type = 'fc' then
    ...
    if p_actor_manager_id is null
       ...
       or not exists (
         select 1 from public.manager_accounts manager
          where manager.id = p_actor_manager_id and manager.active = true
       ) then
      raise exception using errcode = '42501', message = 'invalid_exam_actor';
    end if;
    ...
    if p_actor_admin_id is null
       ...
       or p_actor_fc_id is not null
       or not exists (
         select 1 from public.admin_accounts admin_row
  ...
  if not exists (
    select 1 from public.exam_locations location
     where location.id = p_location_id and location.round_id = p_round_id
  ) then
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(p_fc_id::text || ':' || v_exam_month::text || ':' || v_round.exam_type, 0)
  );
  ...
  if v_registration.id is null then
    insert into public.exam_registrations (
      ...
      v_round.exam_type, 'applied', false, p_includes_primary_exam, p_is_third_exam,
  ...
  insert into public.exam_registration_decision_events (
    registration_id, action, from_status, to_status, actor_type,
    actor_admin_id_snapshot, actor_manager_id_snapshot, actor_fc_id_snapshot,
    target_fc_id_snapshot
  ) values (
    v_registration.id, v_action,
    case when v_action = 'updated' then 'applied' else null end,
    'applied', p_actor_type, p_actor_admin_id, p_actor_manager_id,
    p_actor_fc_id, p_fc_id
  );
