...
export function buildGroupChatAppointmentLabel(member: Extract<GroupChatMemberInput, { kind: 'fc' }>) {
  const lifeCompleted = Boolean(member.life_commission_completed || normalizeText(member.appointment_date_life));
  const nonlifeCompleted = Boolean(member.nonlife_commission_completed || normalizeText(member.appointment_date_nonlife));
...
export function computeGroupChatUnreadCount(input: {
  ...
  const viewerActorId = normalizeText(input.viewerActorId);
  ...
  return input.messages.reduce((count, message) => {
    const senderActorId = normalizeText(message.sender_actor_id);
...
export function computeGroupChatMessageUnreadCounts(input: {
  ...
  const memberActorIds = Array.from(
    new Set(input.members.map((member) => normalizeText(member.actor_id)).filter(Boolean)),
  );
  const readAtByActorId = new Map(
    input.readStates
      .map((row) => {
        const actorId = normalizeText(row.actor_id);
...
export function summarizeGroupChatReactions(input: {
  ...
  input.reactions.forEach((row) => {
    const reaction = normalizeText(row.reaction);
    if (!reaction) return;
...
export function shouldFanoutGroupChatPush(input: {
  ...
  const senderActorId = normalizeText(input.senderActorId);
  const recipientActorId = normalizeText(input.recipientActorId);
...
export function buildGroupChatPreview(input: {
  ...
  if (input.message_type === 'file') return normalizeText(input.file_name) || '파일';
  return normalizeText(input.content) || '메시지';
}
