...
describe('board category contract', () => {
  it('keeps the active board category set to the four GaramIn labels', () => {
    ...
    for (const source of [schema, migration]) {
      ...
      expect(source).toMatch(/'가람pick'\s*,\s*'garam-pick'\s*,\s*4/);
    }
    ...
    expect(categoriesShared).toContain("{ name: '가람pick', slug: 'garam-pick', sortOrder: 4 }");
