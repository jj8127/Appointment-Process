...
- 추천코드가 전달된 본등록은 `apply_referral_link_state`가 성공하기 전까지 `fc_credentials.password_set_at` 또는 `fc_profiles.signup_completed=true`를 쓰면 안 된다. 추천코드가 해석되지 않으면 본등록을 완료시키지 말고 `referral_invalid`로 사용자가 추천인을 다시 선택하게 한다.
