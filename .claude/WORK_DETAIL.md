...
- The existing Expo config plugin now also normalizes the generated `org.gradle.jvmargs` to the proven EAS-parity `4g` heap / `1g` Metaspace contract. Missing, duplicate, or unsupported values fail closed, while EAS worker `GRADLE_OPTS` remains untouched.
