...
- Post-deploy follow-up: manager browser subscriptions hit the existing role check because storage used the session role while staff delivery selects the admin channel. Store the admin delivery channel with the verified personal resident ID; retain manager authorization and FC rejection. Reproduced 500 locally before the fix; after correction, 4 built-route tests and all 576 root tests pass. The root notification test now exercises server-derived scope rather than removed browser implementation strings.
