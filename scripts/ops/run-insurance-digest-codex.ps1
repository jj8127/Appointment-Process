...
Before doing any work, check whether today's KST post titled `보험소식 브리핑 YYYY.MM.DD` already exists in the GaramIn `보험소식` board category. If it exists, verify `fc-notify latest_notice` and FC/admin `inbox_list` include the post, repair only missing notification rows through `fc-notify` if needed, and stop.
