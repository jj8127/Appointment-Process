...
- 2026-05-30 기준 `/dashboard` 모달과 `/dashboard/profile/[id]`의 resident-number birth-date display는 `formatResidentNumberBirthDateDisplay` / `useResidentNumber` helper path를 공유한다. 별도 local birth-date parser를 재도입하지 않는다.
- `use-session` client restore는 cookie-first가 우선이다. middleware cookie session이 유효단면 localStorage fallback을 먼저 읽어 `/dashboard -> /auth` bounce를 만들면 회귀다.
...
- 하단 `관리자 조작` 영역은 좌측 `보증보험 조회 동의일` + 저장, 우측 `사전 심사 요청 하기` + `미승인 / 승인 완료` 토글로 정리되어 있으며, 총무는 `allowance_date`가 있어야 입력 완료/사전 심사/승인 상태를 trusted path에서 저장할 수 있고 본부장은 같은 정보를 read-only로 본다.
- 생명/손해 위촉 탭은 `확정일(Actual)`을 총무가 직접 저장할 수 있는 `완료일 저장` trusted path와, 기존 `일정 저장`/`승인 처리`/`반려` 조작을 함께 제공한다. 직접 저장은 `/api/admin/fc`의 `updateAppointmentDate`로 처리하고, 저장 후 리스트/상세 상태는 `appointment-completed` 또는 `final-link-sent`로 보정한다.
- 생명/손해 위촉 행은 `승인 상태: 승인 완료 / FC 제출, 승인 대기 / 미입력` 배지를 별도로 보여준다. 이미 승인된 항목의 승인 버튼은 `승인 완료됨`으로 비활성화해야 하며, 미승인 항목의 액션 문구는 `승인 처리`로 유지한다.
...
- 생명/손해 위촉 탭 상단의 `최종 완료 상태`는 승인 액션이 아니라 `생명 최종 완료`, `손해 최종 완료` 플래그를 독립 토글로 저장하는 운영 보정 영역이다. 둘 다 꺼진 상태는 별도 버튼 없이 미완료로 본다.
- 보증 보험 동의/서류/다위촉/생명·손해 위촉 반려 사유 입력은 관리자 웹 공통 `RejectReasonModal`을 사용한다. Enter는 반려 제출, Shift+Enter는 줄바꿈이며, 페이지별 Textarea 구현으로 이 동작을 다시 분산하지 않는다.
...
- `/dashboard/referrals/graph`
  - 구조화 추천 관계를 읽기 전용 graph로 탐색단는 화면이다.
  - visible edge는 `recommender_fc_id`를 기본 소스로 만들고, `confirmed attribution`은 같은 edge의 상태를 보강단는 보조 신호로만 겹친다.
  - manager는 진입/조회만 가능하고 mutation CTA는 노출하지 않는다.
  - graph는 Obsidian Graph View의 읽기 경험을 참고단되, 추천인 tree 가독성을 위해 hybrid force-directed 계약을 사용한다. 사용자에게 노출되는 조절값은 `Center force`, `Repel force`, `Link force`, `Link distance` 네 항목이며 localStorage key는 `referral-graph-physics-settings-v14`다.
  ...
  - `배치 초기화`는 hard-pin 해제가 아니라 runtime position을 지우고 component/star/orphan seed layout으로 다시 시작단는 동작이다.
