...
### Staff browser push channel follow-up

`/api/web-push/subscribe` accepts only verified active admin/manager sessions. Both store delivery channel `admin`, matching both staff push sender queries, while `resident_id` stays bound to the verified individual. This is a transport discriminator, not an admin account/session grant. Shared fanout still filters actual non-developer admin accounts; managers/developers receive targeted notifications through their personal ID. FC sessions remain forbidden at this staff subscription endpoint. The fixture reproduces the deployed role-check failure before the change and verifies all staff variants without sending a push or changing a schema.
