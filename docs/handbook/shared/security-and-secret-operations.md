...
- Edge diagnostics for the reviewed auth/referral, Request Board password bridge, notification fanout, Board database/view, and account-cleanup failures terminate at `supabase/functions/_shared/edge-diagnostic.ts`. Its input is a closed event/reason union with only bounded numeric `status`/`count`, boolean `retryable`, and a coarse allowlisted `errorClass`; it cannot accept raw `Error`, message/stack/cause, response body, URL/path, identifier, phone, referral code, or affiliation fields. The runtime reconstructs output field by field and emits a fixed fallback for an invalid event/reason pair.
- The 2026-07-16 reviewed 14-file inventory closes 15 proven sensitive sinks. The remaining direct-console baseline is frozen at exactly 36 entries: 9 fixed non-sensitive literals and 27 unproven investigation items. This baseline is a regression fence, not an approval list; no new, removed, or rewritten sink is permitted without a separate classification and evidence update.
