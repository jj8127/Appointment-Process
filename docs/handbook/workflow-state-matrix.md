...
- 총무는 FC가 수당 동의일을 입력한 뒤에만 수당동의 단계에서 `입력 완료 / 사전 심사 요청 완료 / 승인 완료 / 미승인`을 조작할 수 있고, 본부장은 read-only를 유지합니다. 이때 `status`, `allowance_prescreen_requested_at`, `allowance_reject_reason`이 함께 갱신됩니다.
