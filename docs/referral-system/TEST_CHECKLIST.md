...
- 초대링크 exact 8자리 코드 케이스는 `search-signup-referral` fast path와 signup pending apply single-flight를 함께 본다. 같은 pending code search가 cold/warm start에서 중복 호출되지 않아야 한다.
...
- `RF-DATA-02`는 inviter가 이후 코드를 rotate해도 invitee `fc_profiles.recommender_code` snapshot이 그대로 유지되는지까지 확인해야 PASS다.
...
- `RF-ADMIN-08` 최신 관리자 그래프의 포인터 직접 추적, drag-start 1.2배 링크 제한, 짧은 말단 간격과 긴 child-hub bridge를 검증한다. 실제 브라우저에서 pointer/follower/unrelated drift를 측정하고 release 고정 해제, pan/fit/reset, label, manager read-only 및 콘솔 오류 없음을 확인한다.
...
- `RF-OBS-01` 현재 persisted runtime 범위(`signup_completed`, `referral_linked`, `referral_changed`, `referral_cleared`, `referral_rejected`, code lifecycle, legacy `admin_override_applied`)에서 이벤트 타임라인 복원 가능
- `RF-DATA-01` current-state snapshot과 audit event가 부분 성공 없이 함께 기록되고 inviter snapshot이 유지됨
- `RF-DATA-02` 동명이인 이름과 무관하게 `get_invitee_referral_code(uuid)`가 `fc_profiles.recommender_code` snapshot만 반환하고 inviter 이후 code rotate 영향을 받지 않음
- `RF-DATA-03` `20260423000001_unify_referral_link_state.sql` rollout은 linked DB에 clean apply되고 backfill CTE가 projection 누락 없이 current-state snapshot을 채움
- `RF-HISTORY-01` inviter 계정 삭제 후에도 current-state snapshot/event snapshot으로 이력 복원 가능
...
- 추천인 override는 `before/after/reason/actor`와 함께 `recommender_fc_id` / `recommender_code` snapshot / `referral_events`가 동시에 바뀌는지 확인한다.
- 추천인 clear는 `recommender` 문자열만 있던 레거시 FC에서도 동작해야 하고, clear 이벤트가 추천인 운영 화면 최근 이벤트에 보이는지까지 확인한다.
- `잘못된 자기추천` 제거 케이스는 `recommender_fc_id`와 `recommender_code` snapshot이 null로 정리되고 같은 invitee에 중복 current-state/event가 생기지 않는지 확인한다.
...
- `RF-ADMIN-11` 대량 추천인 조회: 합성 447명 목록/그래프가 40개 이하 ID 필터로 완전 조회되고 중복 이벤트 없이 정렬됨. 로그인 세션 생성, 역할별 알림 조회, 무서명/교차 출처 및 본부장 쓰기 차단도 확인. 운영 배포 상태는 별도 기록.
